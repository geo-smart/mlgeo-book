...
* **COG** (Cloud Optimized GeoTIFF): a GeoTIFF with internal tiling and overviews so clients can request only the tiles they need.
* **GeoParquet**: Parquet with a standardized encoding for vector geometries, replacing shapefiles for large vector datasets.
* **Zarr** (v3): chunked, compressed n-dimensional arrays stored as many small objects, built for parallel cloud reads and the natural cloud sibling of netCDF.
* **kerchunk / VirtualiZarr**: index existing netCDF/HDF5 archives so they can be read as if they were Zarr, without rewriting the data.
* **STAC** (SpatioTemporal Asset Catalogs): a standard JSON catalog for describing and searching geospatial assets, so a script can find the exact scenes or granules it needs.
* **TileDB** and others continue to develop in this space.
...
## Lecture Slides
[Click Here for Lecture Slides](https://docs.google.com/presentation/d/1PVu8vbYtX0G4W41TB537Irm5V845E4uPsIrWQRfoQB0/edit?usp=sharing)
