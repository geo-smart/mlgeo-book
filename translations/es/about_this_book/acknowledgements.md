# Agradecimientos de quienes contribuyeron

Muchas personas dedicadas a la investigación en *machine learning* en las geociencias, y más allá, han contribuido a este libro.

- Marine Denolle (UW, [GitHub](https://github.com/mdenolle))
- Akshay Mehra (UW)
- Ziheng Sun (George Mason University)
- Ariane Ducellier (antes estudiante de posgrado en la UW, [GitHub](https://github.com/ArianeDucellier))
- Stefan Todoran (UW, [GitHub](https://github.com/StefanTodoran))
- Nicoleta Cristea (UW, [GitHub](https://github.com/NCristea))
- Scott Henderson (UW, [GitHub](https://github.com/scottyhq))

El libro se benefició enormemente de otros recursos, entre ellos

- [The Turing Way](https://book.the-turing-way.org/){cite:p}`the_turing_way_community_2022_6909298`
- [Software Carpentries](http://software-carpentry.org/lessons/) 
- El curso [EarthDataScience](https://earthdatascience.org/courses/) de CU Boulder: lecciones adaptadas de materiales desarrollados por Earth Lab
- El [geohackweek](https://geohackweek.github.io/)

## Asistencia de IA

Algunos borradores de tutoriales de la edición 2024 se apoyaron en ChatGPT. La edición 2026 se revisó con la ayuda de un asistente de IA agéntica (Claude), y todo el contenido fue revisado por los instructores. Los conjuntos de datos y el código se atribuyen donde aparecen en el libro.
