...
2. **Clasificación y regresión** (3.2) — los dos tipos de problema supervisado, un primer flujo de trabajo de extremo a extremo y la partición entrenamiento/validación/prueba.
3. **Agrupamiento** (3.3) — descubrimiento no supervisado de estructura: métricas de distancia, k-means desde cero, diagnósticos de silueta y del codo, agrupamiento jerárquico y un ejercicio de sismicidad volcánica.
...
8. **Entrenamiento robusto** (3.8) — validación cruzada para datos correlacionados: por qué las particiones aleatorias mienten en series autocorrelacionadas; particiones temporales, por grupos (sitio, evento) y espaciales dejando un conglomerado fuera; StratifiedGroupKFold para historiales de casos pequeños y desbalanceados; modelos de referencia de persistencia e intervalos de confianza por *bootstrap* sobre los scores.
...
- **Las particiones deben respetar la estructura de los datos.** Las series autocorrelacionadas y los datos espaciales agrupados requieren particiones temporales, por bloques o por grupos (3.8).
...
La lección 3.5 lo lleva a la práctica con una **tabla de clasificación del curso** (*leaderboard*): cada estudiante entrena el clasificador de su elección sobre una partición canónica de un conjunto real de fuentes sísmicas, envía sus predicciones por *pull request* y la integración continua lo califica contra el conjunto de prueba público y contra el oculto.
...
- Diseñar particiones de datos y esquemas de validación cruzada que respeten la correlación temporal y espacial.
