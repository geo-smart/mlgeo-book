...
Español neutro panregional: se fija una variante y se sostiene. Las filas
marcadas ⚑ están en disputa — abra una issue `[translation] term: …`.

| English | Español | ¿EN conservado? | Nota |
|---|---|---|---|
| machine learning | aprendizaje automático | primera mención: «*machine learning* (aprendizaje automático)» | el español domina después |
...
| data leakage | fuga de datos | no | siempre calificada «de datos» |
| spectral leakage | fuga espectral | no | nunca «fuga» sin calificar |
...
| baseline | modelo de referencia | *baseline* tolerado glosado | |
| persistence (forecast) | persistencia | no | |
| feature | característica (variable explicativa) | *feature* tolerado en habla-código | |
| label | etiqueta | no | |
...
| clustering | agrupamiento | *clustering* tolerado glosado | |
...
| deep learning | aprendizaje profundo | *deep learning* tolerado primera mención | |
...
| Brier score | score de Brier | ⚑ | o «puntaje de Brier»; el uso profesional dice «score» |
| coverage (of intervals) | cobertura (empírica) de los intervalos | no | |
| skill (forecast) | — | **sí** (*skill*) glosado «habilidad de pronóstico» | uso operativo |
...
| loss function | función de pérdida | no | |
| gradient descent | descenso de gradiente | no | |
| accuracy | exactitud | no | **nunca «precisión»** — *precision* = precisión |
| precision / recall | precisión / exhaustividad | ⚑ | «recall» tolerado glosado; ⚑ «sensibilidad» circula |
