...
- **Modèles de référence (5 %)** : rapportez vos résultats d'apprentissage automatique classique du jalon précédent (forêt aléatoire, *gradient boosting* ou équivalent). Peu de travail nouveau ; l'objectif est d'avoir une ligne de référence.
...
- **Étude d'ablation (5 %)** — exigée : retirez un composant de votre meilleur modèle (un bloc de couches, le *dropout*, la normalisation par lots, un groupe de caractéristiques ou un terme de perte) et rapportez la variation de performance. Une ablation menée soigneusement suffit ; dites ce qu'elle vous apprend sur le composant.
...
- **Expérience au choix (4 %)** — choisissez-en UNE :
  - *Pré-entraînement contre entraînement à partir de zéro* : pré-entraînez un encodeur sur vos données sans étiquettes (auto-encodeur ou reconstruction masquée, motif du carnet 4.6), puis entraînez un sondage linéaire sur l'encodeur gelé — ou affinez l'encodeur entier — avec une petite fraction étiquetée, et comparez à un entraînement à partir de zéro sur la même fraction. Dites laquelle des deux voies vous avez suivie ; un sondage maintient les poids de l'encodeur fixes, un affinage les met à jour.
