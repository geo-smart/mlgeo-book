...
🖥️ [**Diapositives du cours — Séance 01 (mer. 30 sept.)**](slides/2026/lec01_why_ml_geosciences.html)
...
- **Findable (trouvables)** : les données ont un identifiant pérenne (un DOI), des métadonnées riches, et sont indexées quelque part où l'on peut chercher.
...
- **Le code généré par l'IA n'est pas déterministe.** Posez deux fois la même question au même assistant et vous obtenez deux codes différents. Le seul enregistrement de ce que vous avez réellement exécuté est le code que vous avez validé (*commit*), dans l'environnement que vous avez verrouillé.
