...
🖥️ [**Diapositives du cours — Séance 02 (ven. 2 oct.)**](https://geo-smart.github.io/mlgeo-book/slides/2026/lec02_your_workbench.html)
...
* **Le CLI gh (recommandé).** Installez le [GitHub CLI](https://cli.github.com/) (l'interface en ligne de commande de GitHub) et lancez `gh auth login`. Il vous guide vers une connexion par navigateur et configure les identifiants git pour vous. `gh` sait aussi créer des dépôts (`gh repo create`), ouvrir des *pull requests* (`gh pr create`) et gérer les *issues* depuis le terminal.
...
```
:::{note}
Les GitHub Actions permettent des *workflows* automatisés — les fichiers d'automatisation de l'intégration continue, rangés dans `.github/workflows/` — tels que :
...
- **[`.github/`]** : contient les fichiers propres à GitHub, comme les gabarits d'*issues* et de *pull requests*, et les *workflows* GitHub Actions pour la CI/CD.
...
- **`tests/`** : contient les tests unitaires du paquet. Un *framework* de test comme `pytest` est recommandé.
