...
🖥️ [**Diapositives du cours — Séance 03 (lun. 5 oct.)**](slides/2026/lec03_working_with_agents.html)
...
- **N'acceptez jamais une référence sans la contrôler.** Les assistants fabriquent des références — auteurs plausibles, revue plausible, article inexistant. Confirmez que chaque citation résout vers un document réel qui dit bien ce pour quoi il est cité.
- **N'acceptez jamais une affirmation sur une API sans la contrôler.** Les assistants utilisent avec aplomb des fonctions renommées, dépréciées ou jamais existantes, et leurs données d'entraînement sont en retard sur les versions courantes des bibliothèques (ce livre utilise numpy 2.5 et pandas 3.0 ; une grande partie des données d'entraînement des IA, non). La documentation, pas l'assistant, fait autorité sur une interface.
- **Traitez la relecture par IA comme une première passe, pas comme une autorité.** Demander à un assistant de relire votre code ou votre rapport est réellement utile : il attrape de vrais bogues et de vrais passages obscurs. Mais il encense aussi des conceptions cassées et signale des faux problèmes. Une relecture d'IA est l'avis d'un relecteur qui n'a jamais fait de terrain et ne peut pas rougir. Le devoir du projet final, ci-dessous, rend cela concret.
...
Si vous savez faire ces trois choses, un assistant vous rend plus rapide. Sinon, il vous rend faux à grande échelle.
