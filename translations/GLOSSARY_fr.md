...
chaque chapitre. Les lignes marquées ⚑ sont contestées — ouvrez une issue
`[translation] term: …`.

| English | Français | EN conservé ? | Note |
|---|---|---|---|
| machine learning | apprentissage automatique | première mention : « *machine learning* (apprentissage automatique) » | l'usage français accepte les deux ; le français domine ensuite |
...
| data leakage | fuite de données | non | toujours qualifié « de données » |
| spectral leakage | fuite spectrale | non | toujours qualifié « spectrale » — ne jamais laisser « fuite » nue |
...
| baseline | modèle de référence | *baseline* toléré glosé | |
| persistence (forecast) | persistance | non | usage Météo-France |
| feature | caractéristique (variable explicative) | *feature* toléré dans le code-parlé | |
| label | étiquette | non | |
...
| clustering | partitionnement | *clustering* toléré glosé | ⚑ usage flottant |
...
| Brier score | score de Brier | non | |
| coverage (of intervals) | couverture (empirique) des intervalles | non | |
| skill (forecast) | — | **oui** (*skill*) glosé « score de compétence » | usage opérationnel |
...
| loss function | fonction de perte | non | jamais « fonction de coût » (réservé à *cost*) |
| gradient descent | descente de gradient | non | |
| accuracy | exactitude | non | **jamais « précision »** — *precision* = précision |
| precision / recall | précision / rappel | non | |
